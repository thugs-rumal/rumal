function setPanels(nid) {
    ...
    var $row = $("<tr>")
        .append($("<td>")
            .html("ip"))
        .append($("<td>")
            .html(full_analysis.flat_tree[nid].ip));
    $basic_table.append($row);
    if (full_analysis.flat_tree[nid].certificate) {
        var $row = $("<tr>")
            .append($("<td>")
                .html("certificate"))
            .append($("<td>")
                .html(full_analysis.flat_tree[nid].certificate));
        $basic_table.append($row);
    }
    var $row = $("<tr>")
        .append($("<td>")
            .html("thug verison"))
        .append($("<td>")
            .html(full_analysis.thug.version));
    $basic_table.append($row);
    if (full_analysis.thug.options.proxy) {
        var $row = $("<tr>")
            .append($("<td>")
                .html("proxy"))
            .append($("<td>")
                .html(full_analysis.thug.options.proxy));
        $basic_table.append($row);
    }
    ...
    $("#sidebar > div:nth-child(2) > div")
        .html($("<b>")
            .html("Samples"))
        .append($samples_table);
    $("#sidebar > div:nth-child(2) > div a").click(function() {
        if ($(this).html() === "view") {
            var node = JsonHuman.format(samples[$(this).data("idx")]);
            $("#myModal")
                .modal("show");
            $("#myModal .modal-title")
                .html("Sample");
            ...
            window.open('/raw_content/'+$(this).data("idx")+'/');
        }
    });
            ...
            var $codes_table = $("<table>")
                ...
                .append($("<tr>")
                    .append($("<td>")
                        .html("relationship"))
                    .append($("<td>")
                        .html(codes[i].relationship)));
            var snippet = $("<pre>")
                .addClass("prettyprint linenums")
                .html(codes[i].snippet);
            $("#myModal .modal-title")
                .html("key");
            $("#myModal .modal-body")
                .html($codes_table)
    ...
    $("#sidebar > div:nth-child(4) > div")
        .html($("<b>")
            .html("Plugins"))
        .append($plugins_table);
    $("#sidebar > div:nth-child(4) > div a")
        .click(function() {
            var title = null;
            var body = null;
            if ($(this)
                ...
                title = $(this)
                    .data("key");
                body = JsonHuman.format(full_analysis[$(this)
                    .data("key")]);
            } else {
                var key = $(this)
                    .data("key")
    ...
    $("div.panel:nth-child(7) > div:nth-child(2)")
        .html($threats_table);
    $("div.panel:nth-child(7) > div:nth-child(2) a")
        .click(function() {
